...
try {
  ...
  const { createChipDataset } = await vite.ssrLoadModule('/src/services/export-mcp-chip-dataset.ts');
  await mkdir(outputDirectory, { recursive: true });
  const boardDataset = createBoardDataset(generatedAt);
  const moduleDataset = createModuleDataset(generatedAt);
  const chipDataset = createChipDataset(generatedAt);
  ...
  await writeFile(chipOutputFile, `${JSON.stringify(chipDataset, null, 2)}\n`, 'utf8');
  console.log(`Generated public/api/v1/boards/index.json, board detail files, modules.json (${moduleDatasetBytes.toLocaleString()} bytes; ${moduleDataset.modules.length} module profiles), and chips.json (${chipDataset.chips.length} chip families)`);
