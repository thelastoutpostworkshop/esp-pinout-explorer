...
describe("public board dataset", () => {
  const release = { version: "v0.4.12", source_commit: "a".repeat(40) };
  const dataset = createBoardDataset("2026-01-01T00:00:00.000Z", release);
  const devKitC = dataset.boards.find(
    (board) => board.id === "esp32-s3-devkitc-1",
  );
  ...
  it("emits schema version 1 and required public fields", () => {
    ...
    expect(dataset.release).toEqual(release);
  ...
  it("exports the DevKitC canonical identity and a conservative pin summary", () => {
    expect(devKitC).toMatchObject({
      ...
      module_variants: expect.arrayContaining([
        "ESP32-S3-WROOM-1",
        "ESP32-S3-WROOM-2",
      ]),
      recognition: {
        board_markings: ["ESP32-S3-DevKitC-1"],
        header_pin_counts: [44],
      },
    });
    expect(devKitC?.pinout_summary.safe_general_purpose_pins).toContain(
      "GPIO4",
    );
    expect(devKitC?.pinout_summary.safe_general_purpose_pins).not.toContain(
      "GPIO0",
    );
  });
  ...
  it("derives caution and internal-memory information from profile warnings only", () => {
    expect(devKitC?.pinout_summary.caution_pins).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO0",
          warning: expect.stringContaining("Boot"),
        }),
      ]),
    );
    expect(devKitC?.pinout_summary.reserved_or_internal_pins).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO35",
          reason: expect.stringContaining("PSRAM"),
        }),
      ]),
    );
  });
    ...
    expect(dataset.boards).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          chip_family: "ESP8266EX",
        }),
        expect.objectContaining({
          id: "esp8266-devkits",
          name: "ESP8266-DevKitS",
          chip_family: "ESP8266EX",
        }),
        expect.objectContaining({
          id: "esp-launcher",
  ...
  it("maps the classic WROOM-32 marking to a DevKitC-compatible candidate without changing module variants", () => {
    const devKitCV4 = dataset.boards.find(
      (board) => board.id === "esp32-devkitc-v4",
    );
    expect(devKitCV4).toMatchObject({
      recognition: {
        module_markings: expect.arrayContaining(["ESP-WROOM-32"]),
        header_pin_counts: [38],
      },
...
describe("public module dataset", () => {
  const release = { version: "v0.4.12", source_commit: "a".repeat(40) };
  const dataset = createModuleDataset("2026-01-01T00:00:00.000Z", release);
  const mini1 = dataset.modules.find(
    (module) => module.id === "esp32c3-mini-1",
  );
  ...
  it("exports ESP32-C3-MINI-1 as module-level guidance", () => {
    ...
    expect(mini1?.pin_functions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO4",
          functions: expect.arrayContaining(["MTMS"]),
          warnings: expect.arrayContaining(["JTAG"]),
        }),
      ]),
    );
    expect(mini1?.caution_pins).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO9",
          warning: expect.stringContaining("Boot"),
        ...
        expect.objectContaining({
          gpio: "GPIO18",
          warning: expect.stringContaining("USB"),
        }),
      ]),
    );
    expect(mini1?.general_warnings[0]).toContain("Module-level guidance only");
    expect(mini1?.sources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          url: expect.stringContaining("esp32-c3-mini-1"),
        }),
      ]),
    );
  });
  ...
  it("exports C6 WROOM module profiles with their documented module pads", () => {
    const wroom1 = dataset.modules.find(
      (module) => module.id === "esp32c6-wroom-1",
    );
    const wroom1u = dataset.modules.find(
      (module) => module.id === "esp32c6-wroom-1u",
    );
    ...
    expect(wroom1?.pin_functions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO4",
          functions: expect.arrayContaining(["MTMS"]),
        ...
        expect.objectContaining({
          gpio: "GPIO12",
          functions: expect.arrayContaining(["USB_D-"]),
        }),
        expect.objectContaining({
          gpio: "GPIO17",
          functions: expect.arrayContaining(["U0RXD"]),
        }),
      ]),
    );
    expect(wroom1?.sources).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          url: expect.stringContaining("esp32-c6-wroom-1_wroom-1u"),
        }),
      ]),
    );
...
describe("public raw-chip dataset", () => {
  const release = { version: "v0.4.12", source_commit: "a".repeat(40) };
  const dataset = createChipDataset("2026-01-01T00:00:00.000Z", release);
  ...
  it("exports one source-backed fallback record for every implemented Explorer family", () => {
    ...
    for (const chip of dataset.chips) {
      ...
      expect(chip.sources[0].url).toMatch(
  ...
  it("exports raw ESP32-C6 functions without carrier-board claims", () => {
    ...
    expect(c6?.pin_functions).toEqual(
      expect.arrayContaining([
        expect.objectContaining({
          gpio: "GPIO4",
          functions: expect.arrayContaining(["MTMS"]),
    ...
    expect(p4).toMatchObject({
      ...
      general_warnings: expect.arrayContaining([
        expect.stringContaining(
          "does not yet export a source-backed raw package pin map",
        ),
      ]),
    });
  });
