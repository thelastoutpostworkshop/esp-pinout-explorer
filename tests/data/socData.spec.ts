...
const expectedPinCounts: Record<string, number> = {
  ...
  'esp32c6:esp32c6-wroom-1': 29,
  'esp32c6:esp32c6-wroom-1u': 29,
  'esp32c6:esp32c6-devkitm-1': 30,
  'esp32c6:esp32c6-devkitc-1': 32,
  'esp32c3:esp32c3-qfn32-22-gpio': 33,
  'esp32c3:esp32c3-qfn32-16-gpio': 33,
  'esp32c3:esp32c3-mini-1': 53,
  'esp32c3:esp32c3-mini-1u': 53,
  'esp32c3:esp32c3-wroom-02': 19,
  'esp32c3:esp32c3-wroom-02u': 19,
  'esp32c3:esp32c3-devkitc-02': 30,
  'esp32c3:esp32c3-devkitm-1': 30,
  'esp32c5:qfn48-hr2-hr8': 49,
  'esp32c5:qfn48-hf4': 49,
  'esp32c5:esp32c5-devkitc-1-v1-2': 32,
  'esp32c5:esp-sensairshuttle-v1-0': 19,
  'esp32h2:esp32h2-qfn32': 33,
  'esp32h2:esp32h2-mini-1': 53,
  ...
  'esp32c61:esp32c61-qfn40-hr2-hr8': 41,
  'esp32c61:esp32c61-qfn40-hf4': 41,
  'esp32c61:esp32c61-lga40-nf8r8la': 41,
  'esp32c61:esp32c61-devkitc-1-v2-0': 32,
  'esp32s31:esp32s31-qfn80': 81,
  'esp32s31:esp32s31-function-coreboard-1': 40,
  'esp8285:esp8285-qfn32': 33,
...
describe('SoC data invariants', () => {
    ...
    for (const profile of allProfiles().filter((item) => item.kind === 'board')) {
      ...
      for (const pin of profile.pins) {
        ...
        if (pin.gpio !== undefined) {
          ...
          const hasPackageSourcePins = profile.soc.pins.length > 0 || (profile.soc.packageVariants?.length ?? 0) > 0;
  ...
  it('keeps ESP-Launcher as two 12-pin side headers', () => {
    const profile = allProfiles().find((item) => item.id === 'esp-launcher');
    const leftPins = profile?.pins.filter((pin) => pin.boardHeader === 'J4') ?? [];
    const rightPins = profile?.pins.filter((pin) => pin.boardHeader === 'J2') ?? [];

    expect(profile?.boardLayout).toBe('dual-header');
    expect(leftPins).toHaveLength(12);
    expect(rightPins).toHaveLength(12);
    expect(leftPins.every((pin) => pin.position.side === 'left')).toBe(true);
    expect(rightPins.every((pin) => pin.position.side === 'right')).toBe(true);
    expect(leftPins.map((pin) => pin.position.order).sort((a, b) => a - b)).toEqual(range(1, 12));
    expect(rightPins.map((pin) => pin.position.order).sort((a, b) => a - b)).toEqual(range(1, 12));
  });
