...
const p4EyeSource: SocSource = {
  title: 'ESP32-P4X-EYE User Guide',
  version: 'latest',
  publisher: 'Espressif',
  documentType: 'user-guide',
  url: 'https://docs.espressif.com/projects/esp-dev-kits/en/latest/esp32p4/esp32-p4x-eye/user_guide.html',
  sections: [
    'Getting Started',
  ...
  figures: [
    ...
    },
    {
      ...
      alt: 'ESP32-P4X-EYE female header pin layout',
    ...
    },
    {
      ...
      alt: 'ESP32-P4X-EYE functional block diagram',
...
function buildP4EyeHeaderPin(input: P4EyeHeaderPinInput, resolveSourcePinByGpio: BoardSourcePinResolver): SocPin {
  ...
  return makeBoardPin({
    id: `esp32p4x-eye-header-${input.number}`,
    number: input.number,
    displayNumber,
    label: input.label,
    name: input.label,
    ...
    sourcePin,
    note: `${displayNumber} on the ESP32-P4X-EYE female header, silkscreen label ${input.label}.`,
    notes: input.notes,
...
export function createEsp32p4XEyeBoardProfile(resolveSourcePinByGpio: BoardSourcePinResolver): SocPackageVariant {
  return {
    id: 'esp32p4x-eye',
    name: 'ESP32-P4X-EYE',
    packageName: '2 x 10 female header',
    description:
      'ESP32-P4 vision board with a 2 x 10 female header, 1.54-inch LCD, camera, MicroSD slot, USB device/debug ports, battery connector, and an ESP32-C6-MINI-1U wireless module.',
    kind: 'board',
    ...
    boardSpecs: {
      ...
      onBoardHardware: [
        'ESP32-C6-MINI-1U wireless module, 1.54-inch LCD, MIPI CSI camera, MicroSD card slot, digital microphone, rotary encoder, fill light, battery connector, USB 2.0 Device Port, USB Debug Port, Boot button, Reset button, and SPI flash.',
      ],
    },
    moduleNames: ['ESP32-C6-MINI-1U'],
    moduleVariants: [esp32c6Mini1UModuleVariant],
    identificationNotes: [
      'Choose this profile by the ESP32-P4X-EYE carrier PCB, the ESP32-C6-MINI-1U wireless module, the 2 x 10 female header, and the camera/LCD enclosure markings shown in the official board photos.',
      'The hardware-revision note in the user guide says ESP32-P4X-EYE uses ESP32-P4 chip revision v3.1 or later, unlike the older ESP32-P4-EYE.',
    ],
