...
import {
  getBoardDesignWarnings,
  getMakerWarnings,
  getWarningLabel,
} from "@/data/pinWarnings";
...
export interface BoardIndexDataset {
  ...
  boards: Array<
    ...
    > & {
      pinout_summary: Pick<
        BoardDefinition["pinout_summary"],
        | "safe_general_purpose_pins"
        | "caution_pins"
        | "reserved_or_internal_pins"
        | "peripheral_notes"
        | "general_warnings"
      >;
    }
...
function boardProfile(profileId: string) {
  for (const soc of socs) {
    const profile = soc.boardProfiles?.find(
...
function peripheralNotes(pins: SocPin[]) {
  const usbPins = pins.filter(
    (pin) => pin.gpio !== undefined && hasFunction(pin, /^USB_D[+-]$/),
  );
  const uart0Pins = pins.filter(
    (pin) => pin.gpio !== undefined && hasFunction(pin, /^U0(?:TXD|RXD)$/),
  );
      ...
      ? [
          {
            peripheral: "UART0",
            summary:
              "Header pins with documented UART0 data functions; check the pin cautions before use.",
...
function pinDetails(pins: SocPin[]) {
  return pins
    ...
    .map((pin) => ({
      ...
      board_design_warnings: getBoardDesignWarnings(pin.warnings).map(
        getWarningLabel,
      ),
      notes: pin.notes ?? [],
      analog: pin.analog ?? [],
...
function documentedGpioSharing(pins: SocPin[]) {
  return pins
    .filter(
      (pin) =>
        pin.gpio !== undefined && (pin.warnings ?? []).includes("onboard"),
    )
    .map((pin) => ({
      gpio: gpioName(pin),
      board_label: pin.boardLabel ?? null,
...
function createBoardDefinition(
  ...
  const headerPinCounts = [
    profile.pins.filter((pin) => pin.boardHeader).length,
  ];
  ...
  const reservedPins = gpioPins.flatMap((pin) => {
    const internalWarnings = (pin.warnings ?? []).filter(
      (warning) => warning === "flash" || warning === "psram",
    );
    return internalWarnings.length
      ? [{ gpio: gpioName(pin), reason: warningText(internalWarnings) }]
      : [];
  });
  ...
  return {
    ...
    recognition: {
      ...
      module_markings: unique([
        ...(metadata.moduleMarkings ?? []),
        ...(profile.moduleNames ?? []),
        ...(profile.moduleVariants?.map((variant) => variant.name) ?? []),
      ]),
      ...
      visible_pin_labels: unique(
        profile.pins
          .map((pin) => pin.boardLabel)
          .filter((label): label is string => Boolean(label)),
      ),
      header_pin_counts: headerPinCounts,
      ...(metadata.usbConnectorCount === undefined
        ? {}
        : { usb_connector_count: metadata.usbConnectorCount }),
      usb_connector_types: metadata.usbConnectorTypes,
    },
    pinout_summary: {
      safe_general_purpose_pins: gpioPins
        .filter(isSafeForMakerUse)
        .map(gpioName),
      ...
      general_warnings: unique([
        ...
        ...(cautionPins.length
          ? ["Review each listed caution before wiring a board-header GPIO."]
          : []),
      ]),
    },
    hardware_summary: {
...
export function createBoardDataset(
  ...
  const boards = boardRecognitionMetadata.flatMap((metadata) => {
    ...
    return resolved
      ? [createBoardDefinition(metadata, resolved.soc, resolved.profile)]
      : [];
  });
...
export function createBoardIndexDataset(
  generatedAt = new Date().toISOString(),
  release?: DatasetRelease,
): BoardIndexDataset {
  const { boards } = createBoardDataset(generatedAt, release);
  return {
    schema_version: 1,
    generated_at: generatedAt,
    ...(release ? { release } : {}),
